...
def Fuerza_Stoks(file,path,R,eta):
    ...
    for k in range(len(file)):
        ...
        plt.xlabel('Tiempo [s]')
        plt.title(file[k])
        def fuerza(lista,fps,R,eta):
            cant=[]
            for k in range(len(lista)):
                cant.append(len(lista[k][0]))
            v=np.zeros((np.max(cant)-1,len(lista)))
            ax=np.zeros((np.max(cant)-2,len(lista)))
            for k in range(len(lista)):
                x,y=np.array(lista[k][0])*9.375*1E-8,np.array(lista[k][1])*9.375*1E-8
                v[:len(lista[k][0])-1,k]=np.diff(np.sqrt((x-x[0])**2+(y-y[0])**2))*fps
                ax[:len(lista[k][0])-2,k]=np.diff(np.diff(x-x[0])*fps)*fps
                tiempo = (np.arange(len(lista[k][0])-1)+1)*1/fps
                velx = np.diff(x-x[0])*fps
                plt.plot(tiempo,velx)
            ind = np.unravel_index(np.argmax(np.abs(ax), axis=None), ax.shape)
            vel = np.mean(v[ind[0],np.arange(v.shape[1])!=ind[1]])
            fuer=6*np.pi*eta*R*vel
            return fuer

        fuer = fuerza(lista,15,R,eta)
        F.append(fuer)
        print(f'Fuerza={F[-1]:.3} N')
        plt.show()
    return F
        
# Muestras con silica de 0.5 micrones
#path='Practica 3 Pinzas opticas\\mov browneano muestra2 05um\\'
...
path='Practica 3 Pinzas opticas\\mov_browneano_agua_45um\\'
...
path='Practica 3 Pinzas opticas\\'
